...
seq_len = 6
generator = GeneratorCorrection2(output_dim=1, input_dim=1, hidden_dim=[8, 16, 8], correction_dim = 1, kernel_size=(3, 3), num_layers=3)
...
transforms_ = [
    ...
    transforms.Normalize((0.49,) * (seq_len + 1), (0.5,) * (seq_len + 1))  # (x-mean) / std
]

dataloader = DataLoader(
    ImageDatasetWithSingle(opt.root_dir, opt.img_size, transforms_=transforms_),
...
for epoch in range(start_epoch, opt.n_epochs):
    ...
    while i < len(dataloader):
        ...
        while j < Diters and i < len(dataloader):
            ...
            ''' 1023修改-训练时随机使用一帧纠偏 '''
            random_imgs = sample[1]
            random_imgs = Variable(random_imgs.type(Tensor))
            gen_img = generator(imgs[:, :-1, ...], random_imgs) # x = [bs, 1, 1, h, w]  cor = [bs, 1, h, w]
