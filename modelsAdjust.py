...
class GeneratorNoise(nn.Module):
    def __init__(self, output_dim, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        self.model = nn.Sequential(
            ...
            *upsample(384, 128), # -> 200, layers = 4, and concat noise 96
    ...
    def forward(self, x):
        ...
        output = self.model[26:](output).unsqueeze(1) # [b, 1, 1, h, w], 为了便于后续seq_len维度上concat
...
class DiscriminatorFusion(nn.Module):
    def __init__(self, output_dim, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        ### 最后将通路1、通路2 concat起来
        img_layers.append(nn.Conv2d(2 * output_dim, output_dim, 1, 1, 1))
