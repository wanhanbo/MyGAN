...
parser.add_argument("--cor_path", type=str, default="/path_to_your_pretrained_model.pth", help="path of th correction model")
...
if use_wandb:
    wandb.init(
        ...
        project="sketelonRecallWGan_cor",
...
correction = ConvAutoencoder(output_dim=1, input_dim=1)
# 加载预训练模型的权重文件
correction.load_state_dict(torch.load(opt.cor_path))
# 抽取 encoder 部分
cor_encoder = correction.encoder

discriminator = DiscriminatorFusionCor(output_dim=1, input_dim=1, correction_encoder = cor_encoder, hidden_dim=[8, 8, 8], kernel_size=(3, 3), num_layers=3)
...
def save_sample(batches_done):
    ...
    gen_img = generator(samples[:, :-1, ...], samples[:, -2, ...])
...
for epoch in range(start_epoch, opt.n_epochs):
    ...
    while i < len(dataloader):
        ...
        while j < Diters and i < len(dataloader):
            ...
            ''' 1004修改-短判别器:判别器序列长度从一半开始'''
            short_time_steps = (imgs.shape[1] + 1) // 2
            short_imgs = imgs[:, short_time_steps:, ...]

            ''' 1009修改-训练时使用上一帧纠偏 '''
            gen_img = generator(imgs[:, :-1, ...], imgs[:, -2, ...]) # x = [bs, 1, 1, h, w]  cor = [bs, 1, h, w]
            short_gen_imgs = torch.cat([imgs[:, short_time_steps:-1, ...], gen_img], dim=1)
            i += 1

            real_loss = discriminator(short_imgs)
            fake_loss = discriminator(short_gen_imgs)
            gp = gradientPenalty(short_imgs, short_gen_imgs)
